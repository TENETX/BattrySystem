...
def plot_one_box(kind, x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(
        0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, 10, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        if kind == 1:
            cv2.rectangle(img, c1, c2, color, 30, cv2.LINE_AA)  # filled
            cv2.putText(img, label, (c1[0], c1[1] + 15), 0,
                        0.7, (225, 255, 255), 2, lineType=cv2.LINE_AA)
        else:
            cv2.rectangle(img, (c1[0], c1[1]), (c1[0] + 500,
                          c1[1] - 100), color, -1, cv2.LINE_AA)  # filled
            cv2.putText(img, label, (c1[0], c1[1] - 2),
                        0, tl, (255, 255, 255), 3)


def plot_one(kind, x, img, c=None, ll=None, line=None):
    # Plots one bounding box on image img
    # line/font thickness
    tl = ll or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1
    color = c or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, 10, lineType=cv2.LINE_AA)
    if ll:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(ll, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        if kind == 1:
            cv2.rectangle(img, c1, c2, color, 30, cv2.LINE_AA)  # filled
            cv2.putText(img, ll, (c1[0], c1[1] + 15), 0, 0.7,
                        (225, 255, 255), 2, lineType=cv2.LINE_AA)
        else:
            cv2.rectangle(img, (c1[0], c1[1]), (c1[0] + 500,
                          c1[1] - 100), color, -1, cv2.LINE_AA)  # filled
            cv2.putText(img, ll, (c1[0], c1[1] - 2), 0, tl, (255, 255, 255), 3)
