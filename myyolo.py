...
from login import people
from setting import rmax, rmin, save
import SQLb
import queue
root_path = os.getcwd()
sys.path.insert(0, "./")
oneround = queue.Queue()
# 设置参数
...
def detect(opt, save_img=False):
    ...
    for i in circles[0, :]:
        ...
        for j, det in enumerate(pred):
            ...
            numbers += 1
            SQLb.insert(numr, people, str(numbers) + ".jpg", ret)
            oneround.put([url, [people, numr, numbers]])


def start_process(source, pt):
    SQLb.createone()
